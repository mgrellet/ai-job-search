...
Follow `.claude/skills/job-application-assistant/08-ats-verification.md` in full. It applies to the **CV only**; cover letters rarely go through keyword screening.

**Keyword list:** reuse the required/preferred keywords extracted in Step 1 — do not re-derive them.
...
Run parseability checks and keyword coverage per the framework. For `missing (have it)` items found during `/apply`, add keywords where they fit naturally in the CV, then re-run 5a–5c.

Include the parseability status, keyword coverage table, and top improvements in the Step 6 report (no separate file — unlike `/ats-check`, which saves to `ats-check/`).

**Clean up:** delete the extracted `.txt` file.
